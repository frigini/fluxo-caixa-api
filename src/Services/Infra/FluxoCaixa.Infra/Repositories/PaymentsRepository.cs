...
public class PaymentsRepository(FluxoCaixaContext context) : IPaymentsRepository
{
    ...
    public async Task<IEnumerable<Payment>> GetAll() => await context.Payments.ToListAsync();

    public async Task<IEnumerable<Payment>> GetAllByDate(DateTime date) => await context.Payments.AsNoTracking().Where(p => p.PaymentDate == date).ToListAsync();

    public async Task<IEnumerable<Payment>> GetByType(int type) => await context.Payments.AsNoTracking().Where(p => (int)p.PaymentType == type).ToListAsync();
